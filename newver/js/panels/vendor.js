        ...
        <div class="space-y-8 animate-fadeIn">
            ...
            <section class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <h3 class="text-lg font-bold text-slate-800">My Received Batches & Items</h3>
                        <p class="text-xs text-slate-500">List of all product lots currently or previously routed to your wallet address.</p>
                    </div>
                    <button id="btnRefreshVendorInventory" type="button" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-4 py-2 rounded-xl text-xs transition flex items-center justify-center gap-1.5 cursor-pointer shadow-sm">
                        <i data-lucide="refresh-cw" class="w-3.5 h-3.5"></i> Sync Received Items
                    </button>
                </div>
                        ...
                        <tbody id="tblVendorInventoryBody">
                            <tr>
                                <td colspan="6" class="p-8 text-center text-slate-400 font-medium">
                                    Click "Sync Received Items" to scan the blockchain ledger for your received transactions.
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
async function resolveAccountIdentityRoleText(address) {
    try {
        if (!address || address === "0x0000000000000000000000000000000000000000") {
            return "Unassigned / Void Link Node";
        }
        const identityProfile = await contract.participants(address);
        if (identityProfile && identityProfile.role > 0) {
        ...
        return `Unknown Signer Reference`;
    }
}
...
async function loadVendorTargetBatchAuditLogs() {
    ...
    try {
        ...
        if (batchMetadata.id.toString() === "0") {
        ...
        auditWrapper.innerHTML = `
            <div class="bg-slate-50/40 border border-slate-200/80 rounded-xl p-4 space-y-4 animate-fadeIn">
                <div class="border-b border-slate-200/60 pb-2 flex justify-between items-center">
                    <div>
                        <h4 class="font-bold text-slate-800 text-sm">On-Chain Audit Records: Lot Reference #${batchMetadata.id.toString()}</h4>
                        <p class="text-[11px] text-slate-400 font-medium">Product Item Variety: <strong class="text-slate-700">${batchMetadata.crayfishType}</strong> (${batchMetadata.quantity.toString()} kg)</p>
                    </div>
