...
function renderTransporterView() {
            ...
            <div class="bg-amber-50 border border-amber-200/60 rounded-2xl p-5 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-amber-700 mt-1 leading-relaxed">
                        You temporarily own the item on-chain while it is being physically moved. Update the shipment status to <strong>"In Transit"</strong> upon pickup, and to <strong>"Delivered"</strong> once it arrives at the destination vendor checkpoint.
                    </p>
                </div>
            </div>
    ...
    async function loadAllTransporterAssignedItems() {
        ...
        try {
            // Sequential block lookup through current mapping slots
            while (batchIndexCounter <= 50) {
                const data = await contract.batches(batchIndexCounter);
                
                if (data.id.toString() === "0") break;

                // Only show if transporter matches currentOwner holding target physical assets custody
    ...
    // Auto sync on tab selection switch if wallet state exists
