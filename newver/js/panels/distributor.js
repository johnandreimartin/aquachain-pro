function renderDistributorView() {
    ...
    container.innerHTML = `
        <div class="space-y-8 animate-fadeIn">
            ...
            <div class="bg-violet-50 border border-violet-200/60 rounded-2xl p-5 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-violet-700 mt-1 leading-relaxed">
                        Once the Farmer assigns ownership to your address, you become the tracking node owner. Forward digital custody directly to your Transporter logistics partner so they can manage the shipment phase.
                    </p>
                    <div class="mt-3 flex items-center gap-2 text-xs text-violet-600 font-semibold">
                        ...
                        <i data-lucide="arrow-right" class="w-3.5 h-3.5"></i>
                        <span class="bg-white border border-slate-200 px-2 py-1 rounded-lg">Transporter</span>
                        <i data-lucide="arrow-right" class="w-3.5 h-3.5"></i>
                        <span class="bg-white border border-slate-200 px-2 py-1 rounded-lg">Vendor</span>
                    </div>
                </div>
            </div>
            ...
            <section class="space-y-5">
                <div>
                    <h3 class="text-lg font-bold text-slate-800 mb-1">Forward Batch to Transporter</h3>
                    <p class="text-sm text-slate-500">As a batch owner, transfer custody verification paths downstream to the assigned Transporter node.</p>
                </div>
                <form id="frmDistributorTransfer" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Transporter Wallet Address</label>
                        <input type="text" id="distTransporterTarget" class="form-input-field font-mono" placeholder="0x... destination transporter wallet" required>
                    </div>
                    <div class="sm:col-span-3">
                        <button type="submit" id="btnSubmitDistributorTransfer" class="w-full bg-violet-600 hover:bg-violet-700 text-white font-bold py-3 px-6 rounded-xl text-sm transition flex items-center justify-center cursor-pointer">
                            transferOwnership() → Transporter Node
                        </button>
                    </div>
                </form>
            </section>
    ...
    document.getElementById("frmDistributorTransfer").addEventListener('submit', async (e) => {
        ...
        const target = document.getElementById("distTransporterTarget").value;

        try {
            constructAwaitBlockConfirmationButtonLoader(btn, "");
            const tx = await contract.transferOwnership(id, target);
            await tx.wait();
            alert(`Success: Custody assignment matching Reference Lot #${id} passed downstream to Transporter.`);
            document.getElementById("frmDistributorTransfer").reset();
        } catch(err) {
            alert("Transaction failed: " + (err.reason || err.message));
        } finally {
            constructAwaitBlockConfirmationButtonLoader(btn, "transferOwnership() → Transporter Node");
        }
    });
}